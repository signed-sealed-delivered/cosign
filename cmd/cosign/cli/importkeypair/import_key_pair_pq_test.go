...
func TestImportOfPQKeys(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			createTemporaryPQPrivateKeyForImporting(privateKeyFileName, tt.algorithm, t)

			outputtedKeyPairFileName := "my-pq-test"
			...
			// Clean up: remove temporary PKCS#8 private key
			checkIfFileExistsThenDelete(privateKeyFileName, t)

			// Additional verification: Load the imported key and verify it works before cleanup
			importedKeyPath := outputtedKeyPairFileName + ".key"
			...
			// Verify and clean up: check that encrypted cosign key was created
...
func createTemporaryPQPrivateKeyForImporting(privateKeyName, algorithm string, t *testing.T) {
