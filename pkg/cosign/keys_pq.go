...
// pqKeyPairHandler implements KeyPairHandler with PQ support, chaining to the previous handler for classical keys.
...
func validatePQPublicKey(pub *pqcrypto.PQPublicKey) error {
	...
	case pqcrypto.MLDSA65Algorithm:
		if len(pub.KeyData) != pqcrypto.MLDSA65PublicKeySize {
			return fmt.Errorf("invalid ML-DSA-65 public key size: got %d, want %d", len(pub.KeyData), pqcrypto.MLDSA65PublicKeySize)
		}
	case pqcrypto.MLDSA87Algorithm:
		if len(pub.KeyData) != pqcrypto.MLDSA87PublicKeySize {
			return fmt.Errorf("invalid ML-DSA-87 public key size: got %d, want %d", len(pub.KeyData), pqcrypto.MLDSA87PublicKeySize)
		}
	default:
