...
# 1. Key generation for ML-DSA-65/87 using cosign
...
# Generate ML-DSA-65 keys with cosign
generate_mldsa65_keys() {
    echo_section "Generating ML-DSA-65 Keys with Cosign"

    echo_info "Generating ML-DSA-65 key pair..."
    COSIGN_PASSWORD="" $COSIGN_DIR/cosign generate-key-pair --algorithm ML-DSA-65 --output-key-prefix mldsa65

    if [ -f "mldsa65.key" ] && [ -f "mldsa65.pub" ]; then
        echo_success "ML-DSA-65 key pair generated"
        echo_info "Private key: mldsa65.key ($(wc -c < mldsa65.key) bytes)"
...
# Generate ML-DSA-87 keys with cosign
generate_mldsa87_keys() {
    echo_section "Generating ML-DSA-87 Keys with Cosign"

    echo_info "Generating ML-DSA-87 key pair..."
    COSIGN_PASSWORD="" $COSIGN_DIR/cosign generate-key-pair --algorithm ML-DSA-87 --output-key-prefix mldsa87

    if [ -f "mldsa87.key" ] && [ -f "mldsa87.pub" ]; then
        echo_success "ML-DSA-87 key pair generated"
        echo_info "Private key: mldsa87.key ($(wc -c < mldsa87.key) bytes)"
...
demo_mldsa65_signing() {
    echo_section "Demo 2: ML-DSA-65 Signing"

    echo_info "Signing test-pq.txt with ML-DSA-65 key file..."
    COSIGN_PASSWORD="" $COSIGN_DIR/cosign sign-blob --key mldsa65.key --use-signing-config=false --tlog-upload=false test-pq.txt > test-pq.txt.mldsa65.sig
...
demo_mldsa87_signing() {
    echo_section "Demo 3: ML-DSA-87 Signing"

    echo_info "Signing test-pq.txt with ML-DSA-87 key file..."
    COSIGN_PASSWORD="" $COSIGN_DIR/cosign sign-blob --key mldsa87.key --use-signing-config=false --tlog-upload=false test-pq.txt > test-pq.txt.mldsa87.sig
...
compare_algorithms() {
    ...
    echo ""
    echo "ML-DSA-65:"
    echo "  Private Key:  $(wc -c < mldsa65.key) bytes"
    echo "  Public Key:   $(wc -c < mldsa65.pub) bytes"
    echo ""
    echo "ML-DSA-87:"
    echo "  Private Key:  $(wc -c < mldsa87.key) bytes"
